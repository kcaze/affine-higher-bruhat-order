...
# Principal lower order ideal generated by x
def principal_lower_order_ideal(P: Poset, x: any):
    ret = set()
    todo = set([x])
    while len(todo) > 0:
        x = todo.pop()
...
def delete_congruence_classes(w: Perm, classes: set):
    n = len(w)
    v = [x for x in w if repr(x,n) not in classes]
    for i in range(len(v)):
        x = v[i]
        diff = 0
        for y in classes:
            diff += math.ceil((x - y) / n)
        v[i] = x - diff
    n -= len(classes)
    while sum(v) != n*(n+1)//2:
        if sum(v) < n*(n+1)//2:
...
def generalized_truncation(w: Perm) -> Perm:
    C = congruence_poset(w)
    m = max(maximal_elements(C))
    I = principal_lower_order_ideal(C, m)
    to_delete = set([k for k in C if k not in I])
    to_delete.add(m)
